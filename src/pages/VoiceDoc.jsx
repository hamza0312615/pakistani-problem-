...
export default function VoiceDoc() {
  ...
  const processVoice = async (audioBlob) => {
    ...
    try {
      ...
      let langInstruction = ''
      if (useRomanUrdu) {
        langInstruction = `Translate/generate your compassionate clinical response directly in highly conversational, friendly, and clear Roman Urdu (Urdu language written in standard Latin/English letters, e.g., "Aap ki report ke mutabik sab theek hai. Kisi fikar ki baat nahi hai"). Use simple everyday phrases. Keep it natural, easy to read aloud by an English voice, and extremely concise. Only write in standard Latin letters. Do NOT use Urdu script. Write as a single continuous spoken paragraph without newlines, bullet points, or numbered lists to ensure smooth Text-to-Speech playback.`
      } else if (language.includes('Urdu')) {
        langInstruction = `Provide compassionate, clear, and reassuring first-aid or home care advice directly in extremely clear, polite, and simple conversational Urdu (اردو) script. Use standard everyday Urdu words that are very easy to understand and speak aloud. Avoid difficult or archaic Persian/Arabic medical vocabulary (for example, use 'bukhār' instead of 'tap-e-shuda', 'jild' instead of 'poast', 'āṅkh' instead of 'chashm'). Keep it concise. Write as a single continuous spoken paragraph without newlines, bullet points, or numbered lists to ensure smooth Text-to-Speech playback.`
      } else {
        langInstruction = `Provide compassionate, clear, and reassuring first-aid or home care advice directly in clear, simple, conversational "${language}". Keep sentences simple and easy to understand when spoken aloud. Keep it concise. Write as a single continuous spoken paragraph without newlines, bullet points, or numbered lists to ensure smooth Text-to-Speech playback.`
      }
...
3) Determine the medical urgency level. End your response with exactly one of these urgency flags in English (on a new line): NORMAL, SEE_DOCTOR, or EMERGENCY.`
  ...
  const handleWhatsAppShare = async () => {
    ...
    const text = `*Pakistan Ka Digital Doctor Triage Alert* 🚨\n\n*Symptoms:* "${transcription}"\n*Language:* ${lang}\n*Urgency:* ${urgency}\n\n*AI Advice:* ${advice}\n\n_Sent via Pakistan Ka Digital Doctor_`
